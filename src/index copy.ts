...
export const Config: Schema<Config> = Schema.object({
  autoClearPlay: Schema.number().default(120000).description('指定等待时间未进行自动结束游戏'),
...
export function apply(ctx: Context, config: Config) {
  ...
  const darkEyes = {
    ...
    async readyPlay(session) {
      if (!session.guildId) return { code: false, msg: '请在群内准备游戏！' }
      if (this.playGuild[session.guildId]) return { code: false, msg: '该群已经正在进行游戏，不能重复开启' }
      this.playGuild[session.guildId] = {
        ...
        ready: true, // 准备阶段
        isPlay: false, // 开始阶段
        stopCheck: false, // 是否禁止再查
        playUser: {}, // 所有玩家
        boutTime: null, // 执行周期循环事件
        countdown: null,
        nowTemp: [], // 缓存数据 存活人员
      ...
      const res = this.addPlay(session)
      config.deBug && console.log(this.playGuild);
      return { code: true, msg: `创建成功，已自动为你加入，你将扮演 ${res.name}。其他人请发送 /加入游戏` }
    },
    // 加入游戏
    addPlay(session) {
      if (!session.guildId) return { code: false, msg: '请在群内加入游戏！' }
      if (!this.playGuild[session.guildId]) return { code: false, msg: '当前群内还未开始游戏' }
      if (this.playingUser[session.userId]) return { code: false, msg: '您正在正在进行游戏，不能重复加入' }
      if (this.playGuild[session.guildId]?.isPlay) return { code: false, msg: '游戏已在进行，不能中途加入' }
      if (Object.keys(this.playGuild[session.guildId].playUser).length == 15) return { code: false, msg: '游戏人数已满，最多16人' }
      // 注册捆绑群
      this.playingUser[session.userId] = session.guildId
      // 选择角色进行扮演
      const characters = this.playGuild[session.guildId].characters.pop()
      ...
      this.playGuild[session.guildId].playUser[session.userId] = Object.assign(
        { session: { userId: session.userId, send: (a: string) => null } },
        {
          th_id,
          duty: Duty.civilian, // 身份
          characters, // 虚拟角色信息
          isDie: false, // 是否死亡
          referendum: false, // 是否投票
          isCheck: false, // 是否被调查
          lastWords: '', // 遗言
          isVote: false, // 是否投票
          beVoted: 0, // 被投票数
          killBeVoted: 0, // 杀手投票数
          checkBevoted: 0, // 探员投票数
          hitCount: 1, // 狙杀次数
          healCount: 1, // 治疗次数
          sethealCount: 0, // 被治疗次数
          isHeal: false, // 本回合是否是否被治疗
          isSetHeal: false, // 本回合是否使用针指令
          isSetHit: false, // 本回合是否使用狙指令
          isLastWords: false // 是否发表过遗言
        })
    ...
    clearPlay(session, auto = false) {
      ...
      this.playGuild[session.guildId].closeTime && this.playGuild[session.guildId].closeTime() // 结束准备倒计时
      this.playGuild[session.guildId].countDown && this.playGuild[session.guildId].countDown() // 结束播报倒计时
      ...
      if (!auto) return { code: false, msg: '天黑请闭眼游戏已主动结束' }
    },
    // 开始游戏
    async startPlay(session) {
      ...
      this.playGuild[session.guildId].isPlay = true // 修改状态
      this.playGuild[session.guildId].closeTime && this.playGuild[session.guildId].closeTime() // 清除定时器

      let result = ''
      while (true) {
        this.playGuild[session.guildId].procedure++
        this.playGuild[session.guildId].light = true
        const res = this.battleReport(session)
        await session.send(res.msg)
        ...
        await this.countDown(session, 120) // 开启定时器
        ...
        this.autoBattleReport(session) // 主动推送
        await this.countDown(session, 60, '天亮') // 开启定时器
    ...
    initPlayGame(session, userIdList) {
      ...
      const humanDict = {
        4: { 1: 2, 2: 1, 3: 1 },
        5: { 1: 3, 2: 1, 3: 1 },
        6: { 1: 4, 2: 1, 3: 1 }, // 4平 1探 1强
        7: { 1: 5, 2: 1, 3: 1 }, // 5平 1探 1强
        8: { 1: 4, 2: 2, 3: 2 }, // 4平 2探 2强
        9: { 1: 5, 2: 2, 3: 2 }, // 5平 2探 2强
        10: { 1: 6, 2: 2, 3: 2, }, // 6平 2探 2强
        11: { 1: 5, 2: 2, 3: 2, 4: 1, 5: 1 }, // 5平 2探 2强 1医 1狙
        12: { 1: 6, 2: 2, 3: 2, 4: 1, 5: 1 }, // 6平 2探 2强 1医 1狙
        13: { 1: 5, 2: 3, 3: 3, 4: 1, 5: 1 }, // 5平 3探 3强 1医 1狙
        14: { 1: 6, 2: 3, 3: 3, 4: 1, 5: 1 }, // 6平 3探 3强 1医 1狙
        15: { 1: 5, 2: 4, 3: 4, 4: 1, 5: 1 } // 5平 4探 4强 1医 1狙
      }
      let dictMap = []
      const selectMap = humanDict[userIdList.length] // 选择的游戏队列
      ...
      // 注入职业
      userIdList.forEach((item: string, index: number) => {
    ...
    battleReport_format(referendumList, dieList, lifeList, procedure, session) {
      ...
      const msg = `第${procedure}回合 [${this.playGuild[guild].light ? "白天" : "黑夜"}环节]\n\n` + info +
        `\n---------存活---------\n` +
        lifeList.map((item) => {
          return `${item.th_id}. ${item.characters.name}` +
            (item.beVoted ? ` [被投票数：${item.beVoted}]` : '')
        }).join('\n') +
        '\n-----------------------\n' +
        lineupInfo
    ...
    bandit_battleReport_format(referendumList, dieList, lifeList, procedure, session) {
      ...
      const msg = `第${procedure}回合 [${this.playGuild[guild].light ? "白天" : "黑夜"}环节]\n\n` + info +
        `\n---------存活---------\n` +
        lifeList.map((item) => {
          return `${item.th_id}. ${item.characters.name} ${item.duty === Duty.bandit ? `(${infoRule.dict[Duty.bandit]})` : ''}` +
            (item.killBeVoted ? ` [决策数：${item.killBeVoted}]` : '') + (item.session.userId === session.userId ? ' | 你 |' : '')
        }).join('\n') +
        '\n-----------------------\n' +
        lineupInfo
      return { msg, info: temp }
    ...
    detective_battleReport_format(referendumList, dieList, lifeList, procedure, session) {
      ...
      const msg = `第${procedure}回合 [${this.playGuild[guild].light ? "白天" : "黑夜"}环节]\n\n` + info +
        `\n---------存活---------\n` +
        lifeList.map((item) => {
          return `${item.th_id}. ${item.characters.name} ${(item.isCheck || item.duty === Duty.detective) ? `(${infoRule.dict[item.duty]})` : ''}` +
            (item.checkBevoted ? ` [决定数：${item.checkBevoted}]` : '') + (item.session.userId === session.userId ? ' | 你 |' : '')
        }).join('\n') +
        '\n-----------------------\n' +
        lineupInfo
      return { msg, info: temp }
    ...
    setVote(session, th_id) {
      const res = this.verifyIsPlay(session)
      if (!res.code) return res
      if (!this.playGuild[session.guildId].isPlay) return { code: false, msg: '未开始进行游戏 请先 /开始游戏' }
      if (this.playGuild[session.guildId].playUser[session.userId].isDie) return { code: false, msg: darkEyes.isMe(session) + '你已阵亡，不能参与投票' }
      if (this.playGuild[session.guildId].playUser[session.userId].referendum) return { code: false, msg: darkEyes.isMe(session) + '你已被群众肃清，不能参与投票' }
      if (!this.playGuild[session.guildId].light) return { code: false, msg: darkEyes.isMe(session) + '只有白天才可以参与投票' }

      const goal = this.playGuild[session.guildId].nowTemp.find(item => item.th_id === th_id)
      if (!goal) return { code: false, msg: `没有找到${darkEyes.isMe(session)}你要投票的下标单位` }
      if (session.userId === goal.session.userId) return { code: false, msg: darkEyes.isMe(session) + "不可以投票自己！" }
      if (this.playGuild[session.guildId].playUser[session.userId].isVote) return { code: false, msg: darkEyes.isMe(session) + '你已经投票过了' }
    ...
    cutVote(session, th_id) {
      ...
      if (!this.playGuild[guild]?.isPlay) return { code: false, msg: '未开始进行游戏 请先 /开始游戏' }
      if (this.playGuild[guild].playUser[session.userId].duty !== Duty.bandit) return { code: false, msg: `只有${infoRule.dict[Duty.bandit]}才可以使用 /杀` }
      if (this.playGuild[guild].playUser[session.userId].isDie) return { code: false, msg: darkEyes.isMe(session) + '你已阵亡，不能再行动了' }
      if (this.playGuild[guild].playUser[session.userId].referendum) return { code: false, msg: darkEyes.isMe(session) + '你已被群众肃清，不能再行动了' }
      if (this.playGuild[guild].light) return { code: false, msg: darkEyes.isMe(session) + '只有夜晚才可以参与突袭' }
      const goal = this.playGuild[guild].nowTemp.find(item => item.th_id === th_id)
      if (!goal) return { code: false, msg: `没有找到 ${darkEyes.isMe(session)} 你要突袭的下标单位` }
      if (goal.isDie) return { code: false, msg: `${goal.characters.name} 刚刚已经阵亡。不用杀了` }
      if (session.userId === goal.session.userId) return { code: false, msg: darkEyes.isMe(session) + "不可以投票自己！" }
      if (this.playGuild[guild].playUser[session.userId].isVote) return { code: false, msg: darkEyes.isMe(session) + '你已经选择过了' }
    ...
    checkVote(session, th_id) {
      ...
      if (!this.playGuild[guild]?.isPlay) return { code: false, msg: '未开始进行游戏 请先 /开始游戏' }
      if (this.playGuild[guild].playUser[session.userId].duty !== Duty.detective) return { code: false, msg: `只有${infoRule.dict[Duty.detective]}才可以使用 /查` }
      if (this.playGuild[guild].playUser[session.userId].isDie) return { code: false, msg: darkEyes.isMe(session) + '你已阵亡，不能再查了' }
      if (this.playGuild[guild].playUser[session.userId].referendum) return { code: false, msg: darkEyes.isMe(session) + '你已被群众肃清，不能再查了' }
      if (this.playGuild[guild].light) return { code: false, msg: '只有夜晚才可以参与查' }
      const goal = this.playGuild[guild].nowTemp.find(item => item.th_id === th_id)
      if (!goal) return { code: false, msg: "没有找到你要查的下标单位" }
      if (goal.isDie) return { code: false, msg: `${goal.characters.name} 刚刚已经阵亡。不用调查了` }
      if (session.userId === goal.session.userId) return { code: false, msg: darkEyes.isMe(session) + " 不可以投票自己！" }
      if (this.playGuild[guild].playUser[session.userId].isVote) return { code: false, msg: darkEyes.isMe(session) + ' 你已经选择过了' }
      if (this.playGuild[guild].stopCheck) return { code: false, msg: `允许查的时间过去了，「${infoRule.dict[Duty.detective]}们现在是撤退状态` }
      if (goal.duty === Duty.detective) return { code: false, msg: `ta 是与${darkEyes.isMe(session)}你的同伴，不需要检查！` }
      this.playGuild[guild].playUser[session.userId].isVote = true
      goal.checkBevoted++;
    ...
    hitVote(session, th_id) {
      ...
      if (!this.playGuild[guild]?.isPlay) return { code: false, msg: '未开始进行游戏 请先 /开始游戏' }
      if (this.playGuild[guild].playUser[session.userId].duty !== Duty.sniper) return { code: false, msg: `只有${infoRule.dict[Duty.sniper]}才可以使用 /狙` }
      if (this.playGuild[guild].playUser[session.userId].isDie) return { code: false, msg: '你已阵亡，不能再狙了' }
      if (this.playGuild[guild].light) return { code: false, msg: '只有夜晚才可以狙人' }
      if (this.playGuild[guild].playUser[session.userId].isSetHit) return { code: false, msg: '本夜你已经使用过狙指令！' }
      if (this.playGuild[guild].playUser[session.userId].hitCount == 0) return { code: false, msg: '你已经没有子弹了' }
      const goal = this.playGuild[guild].nowTemp.find(item => item.th_id === th_id)
      if (!goal) return { code: false, msg: "没有找到你要狙的下标单位" }
      if (session.userId === goal.session.userId) return { code: false, msg: darkEyes.isMe(session) + "不可以狙自己！" }
      // 消耗子弹
    ...
    healVote(session, th_id) {
      ...
      if (!this.playGuild[guild]?.isPlay) return { code: false, msg: '未开始进行游戏 请先 /开始游戏' }
      if (this.playGuild[guild].playUser[session.userId].duty !== Duty.medic) return { code: false, msg: `只有「${infoRule.dict[Duty.medic]}」才可以使用 /针` }
      if (this.playGuild[guild].playUser[session.userId].isDie) return { code: false, msg: darkEyes.isMe(session) + '你已阵亡，不能再针了' }
      if (this.playGuild[guild].light) return { code: false, msg: darkEyes.isMe(session) + ' 只有夜晚才可以针人' }
      if (this.playGuild[guild].playUser[session.userId].healCount == 0) return { code: false, msg: darkEyes.isMe(session) + ' 你已经没有针了' }
      const goal = this.playGuild[guild].nowTemp.find(item => item.th_id === th_id)
      if (!goal) return { code: false, msg: darkEyes.isMe(session) + " 没有找到你要针的下标单位" }
      if (goal.isDie) return { code: false, msg: `${goal.characters.name} 刚刚已经阵亡。已经不用治疗了` }
      // 消耗针
    ...
    tallLastWords(session, msg) {
      ...
      const guild = this.playingUser[session.userId]
      if (!this.playGuild[guild]?.isPlay) return { code: false, msg: '未开始进行游戏 请先 /开始游戏' }
      if (!this.playGuild[guild].playUser[session.userId].isDie) return { code: false, msg: '你还未阵亡，无法发表遗言' }
      if (this.playGuild[guild].playUser[session.userId].isLastWords) return { code: false, msg: '你已发表过遗言，无法再次发表遗言' }
      const user = this.playGuild[guild].playUser[session.userId]
      user.isLastWords = true
      const msgg = `[${user.characters.name}]:${msg}`
    ...
    talkUser(session, msg) {
      ...
      if (!this.playGuild[guild]?.isPlay) return { code: false, msg: '未开始进行游戏 请先 /开始游戏' }
      const mayUse = [2, 3] // 允许使用小队对话的单位
      const duty = this.playGuild[guild].playUser[session.userId].duty
      if (!mayUse.includes(duty)) return { code: false, msg: '非阵容人员，无法使用 聊 功能' }
      if (this.playGuild[guild].playUser[session.userId].isDie) return { code: false, msg: '你已阵亡，不能再主动发言了' }
      if (this.playGuild[guild].playUser[session.userId].referendum) return { code: false, msg: '你已被群众肃清，不能再主动发言了' }
      if (this.playGuild[guild].light) return { code: false, msg: '只有夜晚才可以参与对话' }

      // 筛选小队阵容
      const banditList = Object.keys(this.playGuild[guild].playUser).map((item) => {
        return this.playGuild[guild].playUser[item]
      }).filter((item) => {
        if (item.session.userId !== session.userId && item.duty === duty) return true
        return false
      })
    ...
    async settlementReferendum(session) {
      ...
      // 获取票数进行操作
      const bevoteList = lifeList.map((item: any, index: number) => {
        // 投票数记录
        if (item.isVote) isSelectUser++
        return item
      })
    ...
    clerReferendum(session) {
      Object.keys(this.playGuild[session.guildId].playUser).forEach((item) => {
        darkEyes.playGuild[session.guildId].playUser[item].isVote = false; // 重置参与投票
        darkEyes.playGuild[session.guildId].playUser[item].isHeal = false; // 重置本回合被治疗
        darkEyes.playGuild[session.guildId].playUser[item].isSetHit = false; // 重置本回合狙击指令
        darkEyes.playGuild[session.guildId].playUser[item].isSetHeal = false; // 重置本回合治疗指令
        darkEyes.playGuild[session.guildId].playUser[item].beVoted = 0; // 重置被投票数
        darkEyes.playGuild[session.guildId].playUser[item].killBeVoted = 0 // 重置被杀投票数
        darkEyes.playGuild[session.guildId].stopCheck = false // 重置可查操作
      })
    },
    // 验证是否开启游戏
    verifyIsPlay(session, errMsg = '还未开启游戏，请先 /创建游戏') {
      if (!session.guildId) return { code: false, msg: '该功能请在群内操作' }
      if (!this.playGuild[session.guildId]) return { code: false, msg: errMsg }
      if (!this.playGuild[session.guildId].playUser[session.userId]) return { code: false, msg: '你并未参与游戏，无法使用功能' }
      return { code: true, msg: '' }
    },
    // 验证是否开启游戏
    verifyIsPrivatePlay(session, errMsg = '还未创建游戏，请先在群里 /创建游戏') {
      if (session.guildId) return { code: false, msg: '' }
      const guildId = this.playingUser[session.userId]
      if (!this.playGuild[guildId]) return { code: false, msg: errMsg }
      if (!this.playGuild[guildId]?.playUser[session.userId]) return { code: false, msg: '你并未参与游戏，无法使用功能' }
      return { code: true, msg: '' }
    },
    // 更新 session 数据
    ...
    updateGuildId(session) {
      if (session.guildId) {
        if (!this.playGuild[session.userId]) return
